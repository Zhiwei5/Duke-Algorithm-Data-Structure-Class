...
board_t * makeBoard(int w, int h, int numMines) {
  //WRITE ME!
  board_t * p_board = malloc(sizeof(*p_board));
  p_board->height = h;
  p_board->width = w;
  p_board->totalMines = numMines;

  p_board->board = malloc(h * sizeof(int *));
  for (int i = 0; i < h; ++i) {
    (p_board->board)[i] = malloc(w * sizeof(int));
    for (int j = 0; j < w; ++j) {
      (p_board->board)[i][j] = -1;
    }
  }
